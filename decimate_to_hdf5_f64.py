...
def decimate_JCamF(input_file, output_file=None, 
    ...
    file_size_no_header = file_size - np.dtype(dtype).itemsize*header_length - number_of_frames_from_header*(np.dtype(dtype).itemsize*frame_gap)
    print height
    print file_size_no_header
    print np.dtype(dtype).itemsize*int(height)*int(width)
    ...
    for frame_pair_ind in range(number_of_frame_chunks):
        ...
        data = np.fromfile(f,dtype=dtype,count=height*width*temporal_compression+frame_gap*temporal_compression)
        data2=[data[x*(height*width):x*(height*width)+(height*width)] for x in range(temporal_compression)]
        data2=np.ravel(np.array(data2))
        data2 = data2.reshape((temporal_compression,height,width)) + 1 # Corrects for single-bit errors in raw data, with line below
        ...
        for ii in range(spatial_compression):
            for jj in range(spatial_compression):
                for kk in range(temporal_compression): 
                    D += data2[kk, ii::spatial_compression,jj::spatial_compression]
        ...
        # Read and discard spacer between frames:
        data = np.fromfile(f,dtype=dtype,count=frame_gap)
...
for path in JCamfiles:
    ...
    t0 = time.time()
    decimate_JCamF(path, verbose=True,spatial_compression=spatial_compression, \
                    temporal_compression=temporal_compression, transpose=transpose, \
