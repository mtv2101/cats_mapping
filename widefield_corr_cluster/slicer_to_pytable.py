...
def slicer_to_pytable(input_file, save_path, desamp=1):
    ...
    output_name = filename[:-4] + '_' + str(final_size) + 'x' + str(final_size) + '_' + save_name + '.h5'
    ...
    if desamp == 1:
        ...
        start_time = timeit.default_timer()
        for i in range(t):
            mov_h5[i,:,:] = block_reduce(mov[i], block_size=(desamp,desamp), func=np.mean)
        f.close()
        run_time = timeit.default_timer() - start_time
...
if __name__ == "__main__":
    ...
    slicer_to_pytable(r"D:\Cux2_010101JCamF102", r"D:")
    print 'decimating'