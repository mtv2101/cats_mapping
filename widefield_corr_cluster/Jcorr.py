...
class Jcorr(object): 
    ...
    do_rms: if True will calculate the RMS variability of the movie.  
    ...
    def __init__(self,
        ...
        self.maskname = 'mask_256.tif' # mask size should match the final size of the correlation matrix
    ...
    def return_corr(self):        
        ...
        if self.do_bandpass:        
            self.all_corr, self.rms = Jcorr.bandpass_movie(self)        
        else:        
            self.all_corr, self.rms = Jcorr.simple_correlate(self)   
            
        return self.all_corr, self.rms
    ...
    def rms_mov(self, dat):
        #pdb.set_trace()
        rms = np.zeros([self.len_iter])
        rms_img = np.zeros([self.masky*self.maskx])
        yidx = np.array(self.mask_idx[0])
        xidx = np.array(self.mask_idx[1])
        for n in range(self.len_iter):
            wave = dat[:,yidx[n],xidx[n]]/np.mean(dat[:,yidx[n],xidx[n]])-1
            rms[n] = np.sqrt(sum(k*k for k in wave)/len(wave))
        rms_img[self.pushmask] = rms
        rms_img = rms_img.reshape([self.maskx, self.masky])
        return rms_img
    ...
    def bandpass_movie(self):
        ...
        # build corr matrix
        if self.do_rms:
            rms = np.zeros([6, self.masky, self.maskx])
            rms[0,:,:] = Jcorr.rms_mov(self, mov_dff_trunc)
        else:
            rms = []
        ...
        fs = 100.
        bands = [[.1, .3, 2048], # low pass, highpass in Hz, filter length in samples 
                [.3, 1, 1024], 
                [1, 3, 512], 
                [3, 10, 256], 
                [10, 30, 128]]  
        ...
        for nn, band in enumerate(bands):
            ...
            if self.do_rms:
                rms[nn+1,:,:] = Jcorr.rms_mov(self, mov_dff_bp)
    ...
    def simple_correlate(self):
        ...
        if self.do_rms:
            rms = Jcorr.rms_mov(self, mov_dff)
        else:
            rms = []
...
if __name__ == "__main__":  
    ...
    filepath = r'D:\20160226JCamF101_8_8_1.h5'    
    savepath = r'D:'
    ...
    corr = Jcorr(filepath,savepath, 100, 1)
