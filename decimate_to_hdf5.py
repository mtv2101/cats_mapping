...
def decimate_JCamF(input_file, output_file=None, 
    ...
    file_size_no_header = file_size - np.dtype(dtype).itemsize*header_length - NFrames*(np.dtype(dtype).itemsize*frame_gap)
    print height
...
for path in JCamfiles:
    ...
    decimate_JCamF(path, verbose=True,spatial_compression=spatial_compression, \
                    temporal_compression=temporal_compression, transpose=transpose, NFrames=NFrames, \
